...
- **Sifariş yaratma**: istifadəçi `POST /orders {shop_id, items:[{product_item_id, quantity}]}` ilə bir mağazadan bir və ya bir neçə məhsul **variantı** (bax aşağı, "Məhsul variantları") sifariş edir — "Bayraq" yox, məhz "30x60 1 qat". Bütün `items`-in aid olduğu məhsullar eyni `shop_id`-yə aid olmalıdır (qarışıq-mağaza sifarişi yoxdur) — əks halda `400 bad_request`. Qiymət server tərəfdə həll olunur (client-dən etibar edilmir): variant endirimli isə `discount_price`, əks halda `price`. Hər sətir üçün cari **ad və həll olunmuş qiymət sifariş anında "şəkil" kimi saxlanılır** (`order_items.product_name`/`item_name`/`unit_price`) ki, mağaza sonradan qiyməti/endirimi dəyişsə belə köhnə sifariş dəyişməsin.
...
- **Çatdırılma statusu**: `PUT /orders/{id}/status` ilə sifariş `pending → processing → shipped → in_transit → delivered` boru xəttində **yalnız irəliyə** hərəkət edir (geriyə/təkrar `400 bad_request`, irəli atlamaq sərbəstdir). Mağaza (add-product(2)+/admin) istənilən sonrakı mərhələyə keçirə bilər; sifarişin öz müştərisi isə YALNIZ `delivered` təyin edə bilər (çatdığını təsdiqləmək) — hər ikisi eyni son statusa aparır, sistem kimin təyin etdiyini fərqləndirmir.
